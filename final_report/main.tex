...
Fan et al.~\cite{dtcspmm} show $>$2$\times$ from double-buffered
\cpasync{} pipelines for SpMM on Ampere; Chen et al.~\cite{tawa}
sweep depth on H100 revealing occupancy--depth trade-offs; Soi et
...
distance---which we independently confirm on H100.  Zhang et
al.~\cite{hytis} model L2-aware GEMM scheduling but not
async-pipeline interaction.  Swann et al.~\cite{tritonblas} and
...
$S \in \{2, 3, 4\}$).
The V3 source compiles unchanged on all \cpasync{}-capable GPUs;
only \texttt{-arch=sm\_XX} changes.  GEMM ($C = AB$, row-major) is
compute-bound, with arithmetic intensity $T/4$\,FLOP/B growing in
tile size $T$; the 1-D stencil is memory-bound with algorithmic
AI~$\approx$7\,FLOP/B.  Figure~\ref{fig:roofline} plots the
resulting roofline positions.  Kernels live in
\texttt{cuda/src/benchmark\_\{gemm,stencil\}.cu}.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{roofline.png}
\caption{Roofline positions for V0/V1/V3 on all five GPUs.  V0 sits
at AI$\approx$0.25; V1/V3 tiling lifts GEMM AI to $T/4$
(up to $\approx$32); stencil AI is $\approx$7\,FLOP/B.}
\label{fig:roofline}
\end{figure}

\subsection{Sweep and Timing}

GEMM sweeps $N$ in \{256, 512, 1024, 2048, 3072, 4096, 6144, 8192\}
and $T$ in \{16, 32, 64, 128\}.  Stencil sweeps
$L \in \{2^{16},\dots,2^{26}\}$ at tile 256+halo.  Timing uses
\texttt{cudaEvent\_t} with 3 warm-ups + 10 timed iterations (median
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{l2_characterization_v2.png}
...
\centering
\includegraphics[width=0.95\linewidth]{gemm_cross_gpu_best.png}
\caption{GEMM: best V3/V1 speedup vs.\ $N$.}
\end{subfigure}
\vspace{-2pt}
\begin{subfigure}{\linewidth}
\centering
\includegraphics[width=0.95\linewidth]{stencil_speedup_all.png}
...
$\wconc/\cleff$ (in the artefact repository) show the expected
inflection around unity, though L2 alone does not collapse the
curves---A40 and A100-MIG differ by 6.8$\times$ in ridge point.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{pred_vs_meas_scatter.png}
...
A tuned pipeline depth $S$ should be reused across GPUs only when
the ratio $\wconc/\cleff$ falls within the same regime on both GPUs
(i.e.\ $\ll 1$, $\approx 1$, or $\gg 1$).  In particular, any
transfer from H100 to A40 almost always requires re-tuning, and the
Triton pre-filter described in Section~\ref{sec:triton}
automates this check before autotuning begins.

\paragraph{Limitations.}
The hit-fraction $h$ is modelled as a pure capacity ratio, which
ignores set associativity, replacement policy, and writeback
traffic; we believe these omissions account for most of the
residual 9\% GEMM error.  The current formulation also does not
cover DRAM bank conflicts, TMA pipelines (Hopper's successor to
\cpasync{}), or warp-specialised producer/consumer
patterns~\cite{tawa,twill}.  Finally, although the 280-cycle floor
and $\alpha{=}0.15$ behave as universal constants across the tested
GPUs, both were calibrated on a single stencil configuration and
may need re-calibration on post-Hopper architectures.

\paragraph{Summary of findings.}
Our study yields four main findings.  First, pipeline tuning is not
portable across GPUs: on identical source, GEMM speedups span
1.00--1.20$\times$ and stencil speedups span 0.94--1.07$\times$,
and this divergence is driven primarily by L2 capacity and
per-stage overhead.  Second, the H100's nominal 50\,MB L2 yields
only 26\,MB of effective capacity due to slice locality, while the
other tested GPUs match their nominal values; using the 50\,MB
figure on H100 inflates GEMM MAPE from 9.7\% to 10.0\%.  Third, a
two-constant model (a 280-cycle per-stage floor and an occupancy
exponent $\alpha{=}0.15$) combined with pointer-chase microbench
inputs achieves 9.0\% GEMM MAPE and 5.2\% stencil MAPE without any
kernel-data fitting.  Fourth, a Triton pre-filter built on this
model prunes 60--67\% of a 96-configuration GEMM grid; it is
intended as a coarse pre-pass paired with exhaustive benchmarking
of the retained configurations, not as a standalone replacement.

\paragraph{Future work.}
Several directions stand out.  A natural next step is to extend the
model to TMA and warp-specialised pipelines, which replace
\cpasync{} in modern libraries.  A second direction is to combine
the pre-filter with a learned residual, yielding a semi-analytic
autotuner that retains the interpretability of the current model
while reducing the residual error.  A third direction is to apply
the slice-locality insight from the H100 measurement to L2-aware
tile scheduling, potentially recovering more of the nominal L2
capacity for pipelined workloads.

\paragraph{Artefact.}
All code, SLURM scripts, raw CSVs, and figures are released in the
\texttt{EECS-570-Project-26W} repository, and the top-level
\texttt{README.md} documents the end-to-end workflow from
microbenchmark capture to pre-filter deployment.
...
\bibitem{cuda_guide}
{NVIDIA}.
\newblock {CUDA C++ Programming Guide}: Asynchronous Data Copies.
\newblock NVIDIA Documentation, 2025.

\bibitem{cutlass3}
{NVIDIA}.
\newblock {CUTLASS}.
\newblock GitHub repository, NVIDIA/cutlass, 2023.

\bibitem{flashattn3}
Jay Shah et al.
\newblock {FlashAttention-3}: Fast and Accurate Attention with
  Asynchrony and Low-precision.
\newblock In \emph{Advances in Neural Information Processing Systems
  (NeurIPS)}, 2024.

\bibitem{triton}
Philippe Tillet et al.
\newblock Triton: An Intermediate Language and Compiler for Tiled
  Neural Network Computations.
\newblock In \emph{Proceedings of the 3rd ACM SIGPLAN International
  Workshop on Machine Learning and Programming Languages (MAPL)},
  pages 10--19, 2019.

\bibitem{hopper_ieee}
Jack Choquette.
\newblock {NVIDIA Hopper H100 GPU}: Scaling Performance.
\newblock \emph{IEEE Micro}, 43(3):9--17, 2023.

\bibitem{hopper_microbench}
Weile Luo et al.
\newblock Dissecting the {NVIDIA Hopper} Architecture through
  Microbenchmarking and Multiple Level Analysis.
\newblock \emph{arXiv preprint arXiv:2501.12084}, 2025.

\bibitem{dtcspmm}
Ruibo Fan et al.
\newblock {DTC-SpMM}: Bridging the Gap in Accelerating General Sparse
  Matrix Multiplication with Tensor Cores.
\newblock In \emph{Proceedings of the 29th ACM International Conference
  on Architectural Support for Programming Languages and Operating
  Systems, Volume 3 (ASPLOS)}, 2024.

\bibitem{tawa}
Hongzheng Chen et al.
\newblock Tawa: Automatic Warp Specialization for Modern {GPUs} with
  Asynchronous References.
\newblock \emph{arXiv preprint arXiv:2510.14719}, 2025.

\bibitem{twill}
Rupanshu Soi et al.
\newblock Optimal Software Pipelining and Warp Specialization for
  Tensor Core {GPUs}.
\newblock \emph{arXiv preprint arXiv:2512.18134}, 2025.

\bibitem{roofline}
Samuel Williams et al.
\newblock Roofline: An Insightful Visual Performance Model for
  Multicore Architectures.
\newblock \emph{Communications of the ACM}, 52(4):65--76, 2009.

\bibitem{mei_dissecting}
Xinxin Mei et al.
\newblock Dissecting {GPU} Memory Hierarchy Through Microbenchmarking.
\newblock \emph{IEEE Transactions on Parallel and Distributed Systems},
  28(1):72--86, 2017.

\bibitem{gpu_noc}
Zhixian Jin et al.
\newblock Uncovering Real {GPU} {NoC} Characteristics: Implications on
  Interconnect Architecture.
\newblock In \emph{Proceedings of the 57th IEEE/ACM International
  Symposium on Microarchitecture (MICRO)}, pages 885--898, 2024.

\bibitem{hytis}
Zheng Zhang et al.
\newblock {HyTiS}: Hybrid Tile Scheduling for {GPU} {GEMM} with
  Enhanced Wave Utilization and Cache Locality.
\newblock In \emph{Proceedings of the International Conference for
  High Performance Computing, Networking, Storage and Analysis (SC)},
  2025.

\bibitem{tritonblas}
Ryan Swann et al.
\newblock {tritonBLAS}: Triton-based Analytical Approach for {GEMM}
  Kernel Parameter Selection.
\newblock \emph{arXiv preprint arXiv:2512.04226}, 2025.

\bibitem{acta}
Nicol{\'a}s Meseguer et al.
\newblock {ACTA}: Automatic Configuration of the Tensor Memory
  Accelerator for High-End {GPUs}.
\newblock In \emph{Proceedings of the 17th Workshop on General Purpose
  Processing Using GPU (GPGPU)}, pages 21--27, 2025.

\bibitem{layer_conditions}
Dominik Ernst et al.
\newblock Analytical Performance Estimation during Code Generation on
  Modern {GPUs}.
...
\bibitem{mig_guide}
{NVIDIA}.
\newblock {NVIDIA Multi-Instance GPU} User Guide.
